/*
 * File: tests/blurDetector.test.ts
 * Path: C:\\CFH\\frontend\\packages\\inspection-utils\\tests\\blurDetector.test.ts
...
import { runBlurDetection } from '../src/blurDetector';
...
describe('runBlurDetection', () => {
  ...
  it('should fallback to true on TensorFlow error', async () => {
    ...
    const { runBlurDetection: fallbackRun } = await import('../src/blurDetector');
