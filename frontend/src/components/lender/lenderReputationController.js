...
const LenderReputationTracker = ({ lenderId }) => {
  ...
  useEffect(() => {
    const fetchReputation = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/lender-reputation/${lenderId}`);
  ...
  return (
    <Card className="p-4" role="region" aria-label="Lender Reputation Tracker">
      <h3 className="text-lg font-semibold mb-2">Lender Reputation</h3>
      {reputation ? (
        <div className="space-y-2">
          <p className="text-sm text-gray-600">Average Rating</p>
          <p className="text-xl font-semibold">{reputation.rating.toFixed(1)} / 5</p>
          <p className="text-sm text-gray-600">Total Reviews</p>
          <p className="text-xl font-semibold">{reputation.reviews.length}</p>
          <p className="text-sm text-gray-600">Disputes</p>
          <p className="text-xl font-semibold">{reputation.disputes.length}</p>
