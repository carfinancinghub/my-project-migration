/**
 * File: HaulerLoyaltyDashboard.jsx
 * Path: frontend/src/components/hauler/HaulerLoyaltyDashboard.jsx
 * Purpose: Gamified dashboard for haulers to track loyalty points, badges, and job completion rewards
 * Author: SG
 * Date: April 28, 2025
 * Updated: Added SEOHead, used AdminLayout, applied theme.js utilities, enhanced comments and accessibility, corrected export statement
 * Cod2 Crown Certified: Yes
 * Features:
 * - Displays loyalty points summary with total points and job completions
 * - Shows earned badges with animation effects
 * - Tracks job completion rewards progress with accessible progress bars
 * - Responsive layout with TailwindCSS, animations, and error/loading states
 * Functions:
 * - fetchLoyaltyData(): Fetches loyalty data (points, badges, rewards) from /api/hauler/{haulerId}/loyalty
...
const HaulerLoyaltyDashboard = ({ haulerId }) => {
  // State Management
  const [loyaltyData, setLoyaltyData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Fetch Loyalty Data on Component Mount or HaulerId Change
  useEffect(() => {
    const fetchLoyaltyData = async () => {
      try {
        const response = await fetch(`/api/hauler/${haulerId}/loyalty`);
        if (!response.ok) {
  ...
  return (
    <AdminLayout>
      <SEOHead title="Hauler Loyalty Dashboard - CFH Auction Platform" />
      <div
        ...
        <div className="mb-8 animate-fade-in">
          ...
          >
            ...
            <span
              ...
              aria-label="Points earned from job completions"
            >
              Earned from {loyaltyData.jobCompletions} jobs
            </span>
          </div>
        </div>
          ...
          >
            {loyaltyData.badges.length > 0 ? (
              loyaltyData.badges.map((badge) => (
                <div
                  key={badge.id}
                  className={`${theme.successText} bg-green-100 ${theme.spacingSm} rounded-full animate-pulse-short transition-transform hover:scale-105`}
              ...
              <p className={`${theme.fontSizeSm} text-gray-500`} aria-label="No badges earned">
                No badges earned yet. Complete more jobs to unlock!
              </p>
            )}
          </div>
        </div>

        {/* Job Completion Rewards Progress */}
        <div>
...
HaulerLoyaltyDashboard.propTypes = {
  haulerId: PropTypes.string.isRequired,
};

export default HaulerLoyaltyDashboard;