 ...
 * File: AuctionManager.test.ts
 * Path: C:\CFH\backend\tests\auction\AuctionManager.test.ts
 * Purpose: Unit tests for AuctionManager service
 * Author: CFH Dev Team, Grok
 * Date: 2025-07-18 [0803]
 * Version: 1.0.1
 * Version ID: e5f6g7h8-i9j0-abcd-ef12-345678901234
 * Crown Certified: Yes
 * Batch ID: Compliance-071825
 * Artifact ID: e5f6g7h8-i9j0-abcd-ef12-345678901234
 * Save Location: C:\CFH\backend\tests\auction\AuctionManager.test.ts
 */
/**
 * Side Note: TypeScript Conversion & Enhancements
 * - All mocks typed via jest.Mock
 * - Suggest extracting reusable test data to test utils
 * - Suggest adding tests for edge cases (invalid states, permissions)
 * - Suggest: Validate service input using @validation/auction.validation
 */
...
describe('AuctionManager', () => {
  ...
  describe('startAuction', () => {
    it('starts auction successfully', async () => {
      (db.getAuction as jest.Mock).mockResolvedValueOnce({ status: 'pending' });
    ...
    it('returns already active status for active auction', async () => {
      (db.getAuction as jest.Mock).mockResolvedValueOnce({ status: 'active' });
      const result = await AuctionManager.startAuction('789');
  ...
  describe('endAuction', () => {
    it('ends auction successfully as sold', async () => {
      ...
      expect(db.updateAuction).toHaveBeenCalledWith(
        '789',
        expect.objectContaining({ status: 'sold', finalBid: 15000 })
      );
    ...
    it('returns not active status for non-active auction', async () => {
      (db.getAuction as jest.Mock).mockResolvedValueOnce({ status: 'unsold' });
      const result = await AuctionManager.endAuction('789');
  ...
  describe('placeBid', () => {
    it('places bid successfully', async () => {
      ...
      (db.addBid as jest.Mock).mockResolvedValueOnce({});
      const result = await AuctionManager.placeBid('789', '123', 10000);
      expect(result.status).toBe('bid placed');
      expect(db.addBid).toHaveBeenCalledWith(
        '789',
        expect.objectContaining({ bidderId: '123', amount: 10000 })
      );
