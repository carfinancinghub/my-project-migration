/**
 * File: ARCarPreviewGrokOld3.jsx
 * Path: frontend/src/components/buyer/ARCarPreview.jsx
 * Purpose: Enhanced AR car preview with WebXR, interactive customization, tutorial modal, and gamified badges
 * Author: SG
 * Date: April 28, 2025
 */
...
const ARCarPreview = ({ carId, modelUrl, customizationOptions }) => {
  ...
  const [customizationBadges, setCustomizationBadges] = useState(
    JSON.parse(localStorage.getItem('customizationBadges')) || {}
  );
  ...
  // Update customization options with gamification
  const handleCustomizationChange = (key, value) => {
    try {
      setCustomizations((prev) => ({ ...prev, [key]: value }));

      // Award badge for customization (gamification)
      const newBadges = { ...customizationBadges };
      const badgeKey = `${key}-${value}`;
      if (!newBadges[badgeKey]) {
        newBadges[badgeKey] = {
          name: `Customizer (${key}: ${value})`,
          earned: new Date(),
        };
        setCustomizationBadges(newBadges);
        localStorage.setItem('customizationBadges', JSON.stringify(newBadges));
        logger.info(`Badge awarded for customization ${badgeKey}`);
      }
  ...
  return (
    ...
    >
      ...
      {/* Customization badges (gamification) */}
      {Object.keys(customizationBadges).length > 0 && (
        <div className="mb-6">
          ...
          >
            {Object.values(customizationBadges).map((badge, index) => (
              <span
                key={index}
        ...
        >
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            ...
            <p className="text-gray-600 mb-4">
              ...
              <br />
              4. Use customization options to change color and wheels (earn badges!).
              <br />
...
// interactive customization, an accessible tutorial modal, and gamified customization badges,
