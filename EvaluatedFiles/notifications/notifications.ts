...
router.post('/notifications/dispatch', authenticateToken, dispatchLimiter, async (req: Request, res: Response) => {
  try {
...
router.get('/users/me/notifications', authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.put('/users/me/notifications/:notificationId/read', authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.delete('/users/me/notifications/:notificationId', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { notificationId } = req.params;
...
router.post('/batchMarkAsRead', authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.post('/batchDeleteNotifications', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { error } = batchSchema.validate(req.body);
    if (error) {
...
router.post('/users/me/notifications/snooze', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { until } = req.body;
    await notificationService.snooze(req.user.id, until);
    res.status(200).json({ message: Snoozed until  });
  } catch (err) {
...
router.get('/notifications/leaderboards', authenticateToken, async (req: Request, res: Response) => {
  try {
