...
const checkTier = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!Object.values(Tier).includes(tier as string)) {
...
router.post('/reports/custom', checkAuth, checkTier, validate(customReportSchema), async (req: Request, res: Response) => {
    try {
...
router.get('/reports/:reportId/export', checkAuth, checkTier, validate(exportReportSchema), async (req: Request, res: Response) => {
    try {
        const start = Date.now();
        const { userId } = req.user!;
        ...
        const exportData = await analyticsService.exportReport(userId, reportId, format as string);
